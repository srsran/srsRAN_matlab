...
%   returns an OFDM symbol index in a half-frame SSBFIRSTSYMBOLINDEX, given a SSB parttern
%   case SSBPATTERN, a SSB index SSBINDEX and a carrier frequency CARRIERFREQ.
...
function SSBfirstSymbolIndex = srsSSBgetFirstSymbolIndex(SSBpattern, SSBindex, carrierFreq)
    ...
    switch SSBpattern
        case 'B' % 30 kHZ SCS (n = [0] for carrier frequenies <= 3GHz, n = [0, 1] for carrier frequencies > 3 GHz)
            n = [0, 1];
        ...
        case 'E' % 240 kHZ SCS (frequencies larger than 6 GHz only)
            ...
            offset = 56;
        otherwise % case 'A' 15 kHZ SCS, case 'C' 30 kHz SCS (n = [0,1] for carrier frequenies <= 3GHz, n = [0, 1, 2, 3] for carrier frequencies > 3 GHz)
            n = [0, 1, 2, 3];
    ...
    SSBfirstSymbolIndex = SSBfirstSymbolIndexArray(mod(SSBindex, length(SSBfirstSymbolIndexArray)) + 1) + offset * n(floor(SSBindex / length(SSBfirstSymbolIndexArray)) + 1);
